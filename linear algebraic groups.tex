...
Let $\prs{X,A}$ and $\prs{Y,B}$ be affine $\mbb{F}$-varieties. A \emph{regular map} $\varphi \colon \prs{X,A} \to \prs{Y,B}$ is a map of sets $\varphi \colon X \to Y$ such that for every $f \in B$ it holds that $f \circ \varphi \in A$. 
\end{definition}

\begin{definition}[Regular Isomorphism]
A regular map $\varphi \colon \prs{X,A} \to \prs{Y,B}$ is an \emph{isomorphism} if there's $\psi \colon \prs{Y,B} \to \prs{X,A}$ such that $\varphi \circ \psi = \id$ and $\psi \circ \varphi = \id$.
\end{definition}

\begin{remark}
Every homomorphism $\alpha \colon B \to A$ gives a regular map $\varphi \colon X \to Y$ by sending $x$ to the $y \in Y$ for which $\ev_x \circ \alpha = \ev_Y$.
\end{remark}
...
\begin{align*}
\varphi \colon X &\to \mbb{F}^n \\
x &\mapsto \prs{\ev_x \circ \pi\prs{x_i}}_{i \in \brs{n}} \text{.}
\end{align*}
Now $I = \ker\prs{\pi} \subseteq \ker\prs{\ev_p}$ for all $p \in \varphi\prs{X}$, so $I \subseteq I\prs{\varphi\prs{X}}$.
In the other direction, if $f \in I\prs{\varphi\prs{X}}$ then $\ev_x\prs{\pi\prs{f}}$ for all $x \in X$. Therefore $\pi\prs{f} = 0$ so $I\prs{\varphi\prs{X}} \subseteq \ker\prs{\pi} = I$.
Then $A \cong A_n / I\prs{\varphi\prs{X}} = \mbb{F}\brs{\varphi\prs{X}}$.
Then
$\prs{X,A} \cong \prs{\varphi\prs{X}, \mbb{F}\brs{\varphi\prs{X}}}$.

We want $\varphi\prs{X}$ to be closed, which we explain later.
\end{remark}

\begin{exercise}
Check that $\varphi$ in the above remark is injective.
\end{exercise}
