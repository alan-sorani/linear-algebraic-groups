...
\begin{align*}
\varphi \colon X &\to \mbb{F} \\
\prs{x,y} &\mapsto x \text{.}
\end{align*}
The image $\im\prs{\varphi} = \mbb{F} \setminus \set{0}$ is dense, non-closed and open.
\end{example}

\begin{example}
Consider
\begin{align*}
\varphi \colon \mrm{GL}_1\prs{\mbb{R}} \to \mrm{GL}_1\prs{\mbb{R}} \\
x &\mapsto x^2 \text{.}
\end{align*}
We have $\im\prs{\varphi} = \set{y \in \mbb{R}}{y > 0}$ which is (Zariski) dense in $\mrm{GL}_1\prs{\mbb{R}}$ but doesn't contain an open set.
\end{example}

\begin{definition}[Dominant Regular Map]
Let $\varphi \colon X \to Y$ be a regular map between $\mbb{F}$-affine varieties. We say $\varphi$ is \emph{dominant} if $\overline{\varphi\prs{X}} = Y$.
\end{definition}

\subsubsection{Algebraic Interlude}

\begin{proposition}\label{proposition:dominant-injective}
$\varphi \colon X \to Y$ is regular if and only if $\varphi^* \colon \mbb{F}\brs{Y} \to \mbb{F}\brs{X}$ is injective.
\end{proposition}

\begin{proof}
Assume $\varphi$ isn't dominant. We have $Z \coloneqq \overline{\varphi\prs{X}} \subsetneq Y$.
Then there's $f \in \mbb{F}\brs{Y} \setminus 0$ such that $\rest{f}{Z} = 0$. Then $\varphi^*\prs{f} = 0$ so $\ker\prs{\varphi^*} \neq \prs{0}$ so $\varphi^*$ isn't injective.

Assume $\varphi^*$ isn't injective. Then $\ker\prs{\varphi^*} \neq \prs{0}$. Then there's $f \in \mbb{F}\brs{Y} \setminus 0$ such that $\varphi^*\prs{f} = 0$. Then $\varphi\prs{X} \subseteq V\prs{f} \subsetneq Y$ so $\varphi\prs{X}$ isn't dense, a contradiction.
\end{proof}

\begin{theorem}\label{theorem:nonempty_interior}
Let $\varphi \colon X \to Y$ be a dominant regular map between $\mbb{F}$-affine varieties, for $\mbb{F}$ algebraically-closed.
Then $\im\prs{\varphi}$ has non-empty interior.
\end{theorem}

An equivalent statement is the following.

\begin{theorem}
Let $\varphi \colon X \to Y$ be a dominant regular map between $\mbb{F}$-affine varieties, for $\mbb{F}$ algebraically-closed. There's $f \in \mbb{F}\brs{Y} \setminus \set{0}$ such that $\ev_y\prs{f} \neq 0$ implies there's $x \in X$ for which $\ev_x \circ \varphi^* = \ev_y$.
\end{theorem}

\begin{example}
$\varphi \colon \mrm{GL}_1\prs{\mbb{F}} \to \mbb{F}$ sending $x \mapsto x$ gives a map $\varphi^* \colon \mbb{F}\brs{x} \to \mbb{F}\brs{x,x^{-1}}$.
...
\begin{corollary}
Let $\varphi \colon G \to H$ be a homomorphism of algebraic groups over an algebraically-closed field $\mbb{F}$. Then $\im\prs{\varphi}$ is closed.
\end{corollary}
...
\begin{itemize}
\item If $X$ is irreducible, so is $Y$ and we get $\varphi^* \colon \mbb{F}\brs{X} \rmono \mbb{F}\brs{X}$. $X$ is irreducible, hence $\mbb{F}\brs{X}$ is an integral domain. From \ref{proposition:going_up} it follows that there's $f \in \mbb{F}\brs{Y} \setminus \set{0}$ such that for $\eps \colon \mbb{F}\brs{Y} \to \mbb{F}$ satisfying $\eps\prs{f} \neq 0$ there is a lift $\eps' \colon \mbb{F}\brs{x} \to \mbb{F}$.

For $y \in Y$ with $\ev_y\prs{f} = f\prs{y} \neq 0$ we can find $\eps' \colon \mbb{F}\brs{X} \to \mbb{F}$ lifting $\ev_y$. But, $\eps' = \ev_x$ for some $x \in X$. We get that $\ev_y = \ev_x \circ \varphi^*$, which is equivalent to $\phi\prs{x} = y$. Then $V_f \subseteq \varphi\prs{X}$.

\item Assume $X = \bigcup_{i \in [s]} X_i$ is a decomposition of $X$ to irreducible components. It follows from the irreducible case that there are open sets $U_1, \ldots, U_s \subseteq Y$ such that $\overline{\varphi\prs{X_i}} \cap U_i \subseteq \varphi\prs{X_i}$ for all $i \in [s]$. Write
$U \coloneqq \bigcap_{i \in [s]} U_i'$ for $U_i' = U_i \cup \prs{Y \setminus \overline{\varphi\prs{X_i}}}$. For every $i \in [s]$ we get
\[\overline{\varphi\prs{X_i}} \cap U \subseteq \overline{\varphi\prs{X_i}} \cap U_i' \subseteq \varphi\prs{X_i} \text{.}\]
Also, $U \subseteq \bigcup_{i \in [s]} \varphi\prs{X_i} = \varphi\prs{X}$.
...
\begin{proposition}
If $\varphi \colon G \to H$ is a regular homomorphism of algebraic groups, $\varphi\prs{G}$ is a closed subgroup of $H$.
\end{proposition}

\begin{proof}
$\varphi\prs{G} \leq H$ is a subgroup, and from \ref{theorem:nonempty_interior}, $\varphi\prs{G}$ contains an open subset of $\overline{\varphi\prs{G}}$. From the previous proposition we get that $\varphi\prs{G}$ is closed.
\end{proof}
