...
\begin{lemma}
Let $\mbb{F}$ be an algebraically-closed field. Let $A \rmono B$ be a subring of a reduced ring $B$ such that $B$ is generated by $A$ and $t \in B$. Write $B \cong A\brs{t} / I$. Assume there are a homomorphism $\eps \colon A \to \mbb{F}$ and an element $f \coloneqq f_0 + f_1 t + \ldots + f_n t^n \in I$ such that $\eps\prs{f_n} \neq 0$.
...
\begin{proposition}
Let $\mbb{F}$ be an algebraically-closed field. Let $B$ be a finitely-generated $\mbb{F}$-algebra, which is an integral domain.
